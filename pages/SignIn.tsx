  ...
  return (
    <div className="flex items-center justify-center py-12 px-4 w-full min-h-[80vh] relative z-0">
      <div className="w-full max-w-md relative">
        ...
        {/* Decorative Elements behind card - Added pointer-events-none to prevent blocking clicks */}
        <div className="absolute -top-20 -left-20 w-64 h-64 bg-primary-500/20 rounded-full blur-[100px] animate-pulse pointer-events-none"></div>
        <div className="absolute top-40 -right-20 w-64 h-64 bg-blue-500/20 rounded-full blur-[100px] animate-pulse animation-delay-2000 pointer-events-none"></div>

        {/* Card */}
        <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-white/20 dark:border-slate-700/50 shadow-2xl rounded-3xl overflow-hidden ring-1 ring-slate-900/5 dark:ring-white/10 animate-fade-in-up z-10">

          {/* Top Bar Design */}
          <div className="h-1.5 bg-gradient-to-r from-primary-500 via-purple-500 to-blue-500 w-full"></div>
          ...
          <div className="p-8 md:p-10">
            <div className="text-center mb-10">
              ...
              <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{t('auth.signIn.title')}</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 font-medium">{t('auth.signIn.subtitle')}</p>
              {error && (
            ...
            <form onSubmit={handleSignIn} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 ml-1">{t('auth.signIn.emailLabel')}</label>
              ...
              {/* Password Input */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 ml-1 flex justify-between">
                  <span>{t('auth.signIn.passwordLabel')}</span>
                  <button type="button" className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 lowercase font-bold text-[10px] bg-primary-50 dark:bg-primary-900/20 px-2 py-0.5 rounded-full transition-colors">
              ...
              >
                {isLoading ? (
                  <><Loader2 size={20} className="animate-spin" /> {t('auth.signIn.submitting')}</>
                ) : (
                  <><span>{t('auth.signIn.submit')}</span><ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></>
                )}
              </button>
            </form>
            ...
            <div className="text-center mt-8 pt-2">
              <p className="text-slate-500 dark:text-slate-400 text-sm">
                ...
                <button onClick={() => navigate('/signup')} className="text-primary-600 dark:text-primary-400 font-bold hover:underline decoration-2 underline-offset-4">
